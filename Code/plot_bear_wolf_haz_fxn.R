plot_bear_wolf <- function(mod, xlabel, ylabel){
...
mean.b <- mod$BUGSoutput$mean$Prob.yr.b
mean.w <- mod$BUGSoutput$mean$Prob.yr.w

nyear <- length(mean.b)
b.ci <- apply(mod.pars$Prob.yr.b,2,"quantile",probs=c(0.025,0.25,0.75,0.975))
w.ci <- apply(mod.pars$Prob.yr.w,2,"quantile",probs=c(0.025,0.25,0.75,0.975))

plot(mean.w[26:(nyear-1)], mean.b[26:(nyear-1)], xlim = c(0,1), ylim = c(0,.3), 
